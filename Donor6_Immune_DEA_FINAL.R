...
data_path <- paste(getwd(),"Donor_6_MS_Immune_Compartment_FINAL.rds",sep="\\")
dataset <- "Donor_6_MS_Immune_Compartment_FINAL"
data <- readRDS(data_path)

# Set the active ident to be the cell type and reorder for figure generation
Idents(data) <- data@meta.data$cell_type
data$cell_type <- factor(data$cell_type,levels=c("Mast","NK","T","PMN","Mono","DC","MP","Plasma","B"))
...
#ggsave("donor6_immune_umap_SC.jpg",plot=sc,width=20,height=20,units="cm",dpi=600)
#ggsave("donor6_immune_umap_SN.jpg",plot=sn,width=20,height=20,units="cm",dpi=600)

# DEA
out_dir = ("C:\\Users\\BASanto\\Desktop\\GUDMAP_Manuscripts\\scRNAseq_v_snRNAseq_MS\\Making_Final_UMAPs\\Immune_DEA")
...
Idents(data) <- data@meta.data$cell_type
data_subset <- subset(data,idents=c("MP","Mono","T"))
...
data_subset$cell_type <- factor(data_subset$cell_type,levels=c("MP","Mono","T"))

cmap <- c("#3366CC","#003399","#CC99FF") # no B, Plasma, or PMN
...
sn_master_map
#ggsave("donor6_immune_DEA_SN_15Log2FC_PctDiff.jpg",plot=sn_master_map,width=20,height=20,units="cm",dpi=600)
...
sc_master_map
#ggsave("donor6_immune_DEA_SC_15Log2FC_PctDiff.jpg",plot=sc_master_map,width=20,height=20,units="cm",dpi=600)
